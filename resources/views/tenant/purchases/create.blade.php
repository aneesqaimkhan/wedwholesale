...
@section('content')
    ...
    <div class="card compact-form">
        <form method="POST" action="{{ route_include_subdirectory('purchases.store') }}">
            @csrf
            ...
            <!-- Supplier, Date, Remarks in one row -->
            <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 8px; margin-bottom: 12px;">
                <div class="form-group">
                    <label>Search Supplier *</label>
                    <input list="supplier_list" class="form-control" id="supplier_input" placeholder="Type supplier name to search" required>
                    <datalist id="supplier_list">
                        @foreach($suppliers as $s)
                            <option value="{{ $s->name }}" data-id="{{ $s->id }}" data-name="{{ $s->name }}" data-mobile="{{ $s->mobile ?? '' }}" data-address="{{ $s->address ?? '' }}"></option>
                        @endforeach
                    </datalist>
                    <input type="hidden" name="supplier_code" id="supplier_code" required>
            ...
            <div style="display: flex; gap: 8px; margin-top: 10px; justify-content: flex-end;">
                <button type="submit" class="btn" id="submit_btn" style="padding: 6px 16px; font-size: 12px;">Save Purchase</button>
            </div>
        </form>
    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Clear supplier error on change
        ...
        document.querySelector('form').addEventListener('submit', function(e) {
            ...
            const supplierInput = document.getElementById('supplier_input');
            if (!supplierCode.value || supplierCode.value.trim() === '' || 
                !supplierName.value || supplierName.value.trim() === '') {
                errors.push('Supplier is required');
                supplierInput.style.borderColor = '#dc3545';
                supplierInput.style.borderWidth = '2px';
            } else {
                supplierInput.style.borderColor = '';
                supplierInput.style.borderWidth = '';
            }
            ...
            if (errors.length > 0) {
                ...
                const firstError = invoiceDate.style.borderColor === 'rgb(220, 53, 69)' ? invoiceDate :
                                  supplierInput.style.borderColor === 'rgb(220, 53, 69)' ? supplierInput :
    ...
    <datalist id="product_list">
        @foreach($products as $p)
            <option value="{{ $p->product_code }} - {{ $p->product_name }} (Retail: {{ number_format($p->r_price_box ?? 0, 2) }})" 
    ...
    <script>
        (function() {
            const suppliers = document.getElementById('supplier_list').options;
            const supplierInput = document.getElementById('supplier_input');
            const codeEl = document.getElementById('supplier_code');
            const nameEl = document.getElementById('supplier_name');
            const addrEl = document.getElementById('address');
            supplierInput && supplierInput.addEventListener('change', function() {
                const val = this.value;
                for (let i = 0; i < suppliers.length; i++) {
                    if (suppliers[i].value === val) {
                        codeEl.value = suppliers[i].dataset.id || '';
                        nameEl.value = suppliers[i].dataset.name || '';
                        addrEl.value = suppliers[i].dataset.address || '';
                        break;
                    }
                }
            });
            ...
            function wireRow(tr) {
                ...
                function selectProduct(productOption) {
                    ...
                    // Show product code, name with retail price in product field
                    pInput.value = code + ' - ' + name + (rPrice > 0 ? ' (Retail: ' + rPrice.toFixed(2) + ')' : '');
                ...
                pInput.addEventListener('change', function() {
                    ...
                    for (let i = 0; i < products.length; i++) {
                        if (products[i].value === val || products[i].dataset.name === val) {
                    ...
                    // If no exact match, try partial match (for when user types product name)
                    if (!pCode.value && !pName.value) {
                        for (let i = 0; i < products.length; i++) {
                            const productName = products[i].dataset.name || '';
                            if (val && productName.toLowerCase().includes(val.toLowerCase())) {
                                selectProduct(products[i]);
                                // Clear items table error border when product is selected
                                const itemsTable = document.getElementById('items_table');
                                if (itemsTable) {
