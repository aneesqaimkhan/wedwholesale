...
<div class="card compact-form">
    <form method="POST" action="{{ route_include_subdirectory('salesmen.store', ['subdomain' => request()->route('subdomain')]) }}">
        @csrf
        
        <div class="grid-2">
            <div class="form-group">
                <label for="name">Salesman Name *</label>
                <input type="text" id="name" name="name" class="form-control" value="{{ old('name') }}" placeholder="Enter salesman name" required>
                @error('name')
        ...
        <div style="display: flex; gap: 8px; margin-top: 10px;">
            <button type="submit" class="btn" style="padding: 6px 16px; font-size: 12px;">Create Salesman</button>
