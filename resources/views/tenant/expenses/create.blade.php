...
<div class="card compact-form">
    <form method="POST" action="{{ route_include_subdirectory('expenses.store', ['subdomain' => request()->route('subdomain')]) }}">
        @csrf
        ...
        <div class="form-group">
            ...
            <input type="number" id="amount" name="amount" class="form-control" value="{{ old('amount') }}" step="0.01" min="0" placeholder="0.00" required>
            @error('amount')
                <div class="error" style="color: #dc3545;">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <textarea id="remarks" name="remarks" class="form-control" placeholder="Enter remarks (optional)">{{ old('remarks') }}</textarea>
            @error('remarks')
                <div class="error" style="color: #dc3545;">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div style="display: flex; gap: 8px; margin-top: 10px;">
            <button type="submit" class="btn" style="padding: 6px 16px; font-size: 12px;">Create Expense</button>
