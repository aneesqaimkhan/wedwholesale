...
<div class="card compact-form">
    <form method="POST" action="{{ route_include_subdirectory('expense_types.update', ['subdomain' => request()->route('subdomain'), 'expense_type' => $expenseType->id]) }}">
        @csrf
        @method('PUT')
        
        <div class="form-group">
            <label for="name">Expense Type Name *</label>
            <input type="text" id="name" name="name" class="form-control" value="{{ old('name', $expenseType->name) }}" placeholder="Enter expense type name" required>
            @error('name')
        ...
        <div style="display: flex; gap: 8px; margin-top: 10px;">
            <button type="submit" class="btn" style="padding: 6px 16px; font-size: 12px;">Update Expense Type</button>
