...
@section('page-title', 'Customer List Report')

@section('content')
<div class="card">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
        <div>
            <h3 style="margin: 0; font-size: 16px; font-weight: 600;">Customer List Report</h3>
            <p style="margin: 5px 0 0 0; color: #666; font-size: 12px;">Complete customer directory</p>
        </div>
        <a href="{{ route_include_subdirectory('reports.index') }}" class="btn" style="padding: 6px 16px; font-size: 12px;">← Back to Reports</a>
    </div>

    <!-- Filters -->
    <form method="GET" action="{{ route_include_subdirectory('reports.customer_list') }}" style="background: #f8f9fa; padding: 15px; border-radius: 5px; margin-bottom: 15px;">
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px;">
            <div class="form-group" style="margin-bottom: 0;">
                <label style="font-size: 12px; margin-bottom: 5px;">Area</label>
                <select name="area_id" class="form-control" style="font-size: 12px; padding: 8px;">
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $data['last_purchase_date'] ?? 'N/A' }}</td>
                        <td style="font-weight: 600; {{ $data['outstanding_balance'] < 0 ? 'color: red;' : ($data['outstanding_balance'] > 0 ? 'color: green;' : '') }}">
