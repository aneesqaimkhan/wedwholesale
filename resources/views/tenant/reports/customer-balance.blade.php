...
<div class="card">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
        <div>
            <h3 style="margin: 0; font-size: 16px; font-weight: 600;">Customer Balance Report</h3>
            <p style="margin: 5px 0 0 0; color: #666; font-size: 12px;">Customer account balances</p>
        </div>
        <a href="{{ route_include_subdirectory('reports.index') }}" class="btn" style="padding: 6px 16px; font-size: 12px;">← Back to Reports</a>
    </div>

    <!-- Filters -->
    <form method="GET" action="{{ route_include_subdirectory('reports.customer_balance') }}" style="background: #f8f9fa; padding: 15px; border-radius: 5px; margin-bottom: 15px;">
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ number_format($data['total_receipts'], 2) }}</td>
                ...
                <tfoot>
                    <tr style="font-weight: 600; background: #f8f9fa;">
                        ...
                        <td>{{ number_format(array_sum(array_column($balanceData, 'total_sales')), 2) }}</td>
                        <td>{{ number_format(array_sum(array_column($balanceData, 'total_receipts')), 2) }}</td>
    ...
    @else
        <div style="text-align: center; padding: 30px; color: #666;">
            <p>No customer balances found matching the criteria.</p>
        </div>
    @endif
</div>
@endsection
