...
@section('content')
<div class="card">
    <form method="POST" action="{{ route_include_subdirectory('roles.store', ['subdomain' => request()->route('subdomain')]) }}" class="compact-form">
        @csrf
                        ...
                        <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(250px, 1fr)); gap: 5px;">
                            @foreach($modulePermissions as $permission)
                                <label style="display: flex; align-items: center; font-size: 11px; cursor: pointer; padding: 4px;">
                                    <input type="checkbox" name="permissions[]" value="{{ $permission->id }}" style="margin-right: 6px;">
        ...
        <div style="margin-top: 20px; display: flex; gap: 10px;">
            <button type="submit" class="btn" style="padding: 8px 20px; font-size: 12px;">Create Role</button>
